...
Minimal Gazebo warehouse (≈8 m × 10 m interior) for TurtleBot3 + SLAM + Nav2 + fake semantic navigation.
...
## Launch with TurtleBot3 (ROS 2 Jazzy)

**Recommended — package launch** (uses `warehouse_semantic.world` + gz-sim server/GUI + spawn + `ros_gz` bridges):
...
Add `use_gzclient:=false` for a headless run.

Note: upstream `turtlebot3_gazebo/empty_world.launch.py` **hardcodes** its world path; it does not accept `world:=...`. Any world in this package should go through `tb3_sim.launch.py`, which is what the launch above wraps. To drive gz-sim by hand instead:

```bash
WORLD=$(ros2 pkg prefix tb3_frontier_exploration)/share/tb3_frontier_exploration/worlds/warehouse_semantic.world
ros2 launch ros_gz_sim gz_sim.launch.py gz_args:="-r -s -v2 $WORLD"
# other terminal:
ros2 launch ros_gz_sim gz_sim.launch.py gz_args:="-g -v2"
```

Spawning the robot that way also needs `ros_gz_sim create` plus the
`ros_gz_bridge`/`ros_gz_image` bridges — see `launch/tb3_sim.launch.py`.
Do not use `turtlebot3_gazebo/spawn_turtlebot3.launch.py` here: its bridge
config maps `/cmd_vel` as `TwistStamped`, which this project does not use.
...
Requires `GZ_SIM_RESOURCE_PATH` to include
